...
cache_dir="$HOME/.cache/awww/$awww_version/"
rofi_link="$HOME/.config/rofi/.current_wallpaper"
wallpaper_current="$HOME/.config/hypr/wallpaper_effects/.wallpaper_current"
read_cached_wallpaper() {
  local cache_file="$1"
  if [[ -f "$cache_file" ]]; then
    # awww cache might be different, for now we fallback to query if empty
...
cp -f "$wallpaper_path" "$HOME/.config/hypr/wallpaper_effects/.wallpaper_modified" || true
