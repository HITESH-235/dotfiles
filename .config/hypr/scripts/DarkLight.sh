...
# Function to apply persistent Waybar size and color overrides
# Uses an import to ensure settings stick across theme switches
patch_waybar_size() {
    waybar_style_link="$HOME/.config/waybar/style.css"
    real_file=$(readlink -f "$waybar_style_link")
    
    if [ -z "$real_file" ] || [ ! -f "$real_file" ]; then
        echo "Cannot resolve waybar style file for patching."
        return
    fi

    # Ensure our overrides are imported at the end of the file
    # We check if it's already there first to avoid duplicates
    if ! grep -q "user-overrides.css" "$real_file"; then
        echo '@import "/home/hit235/.config/waybar/user-overrides.css";' >> "$real_file"
    fi
}

# Call the function after determining the mode
# (Commented out to prevent theme switches from changing the custom Waybar style)
# set_waybar_style "$next_mode"
# Apply persistent size overrides to the newly selected style
patch_waybar_size
